...
## ✨ Key Features
* **Zero Setup Required:** Automatically downloads the latest Google Platform Tools (ADB/Fastboot) and `payload-dumper-go` the first time you run it.
...
* **Automated ROM Flashing:** Automatically extracts `payload.bin`, dumps the required boot images, flashes them, and initiates an ADB sideload in one seamless sequence.
...
### 5. Flash Full Custom ROM (The Big One)
...
* **In Action:** 
  ...
  2. The tool uses native Windows `tar` to crack open the `.zip` and extract the `payload.bin` file.
...
### 7-9. Fastboot Wiping (Miscellaneous)
* **Erase FRP:** Executes `fastboot erase frp` to wipe the Factory Reset Protection partition.
* **Erase Metadata:** Executes `fastboot erase metadata` to clear encryption states.
* **Erase Userdata:** Executes `fastboot erase userdata` to completely format your internal storage (essential when clean-flashing a new ROM).

### 10. Mi Unlock Tool (Python)
